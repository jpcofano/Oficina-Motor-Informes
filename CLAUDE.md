...
- ⭐⭐ **Y el corolario que vale para todo instrumento nuevo, no sólo para éste: no reporta hasta que se sabe qué devuelve sobre un CASO CONOCIDO.** Es el segundo lector de más arriba aplicado a un instrumento en vez de a dos lecturas — **un detector sin control positivo no distingue un problema real de una limitación propia**. Medido el 30/08: un detector escrito para nombrar **un** marcador que falla devolvió **42**, casi todos falsos por defectos del propio parser. ⚠ **Publicarlos habría costado más que el bug que se buscaba**, porque alguien los habría trabajado uno por uno. La salida fue **preguntarle al motor** en vez de reimplementar su lógica — y que la respuesta **declare el conteo aunque dé cero**, porque un cero declarado es un dato y un cero silencioso no.
...
⛔⛔ **Un parser que no falla y devuelve la columna de al lado. Y su familia: el lector que entrega datos PLAUSIBLES en vez de un error.** (30/08/2026.) Es el mismo modo de falla que el `String(celda)` de abajo —instrumento propio que reproduce peor lo que ya existía— pero **un escalón antes: acá se rompe la LECTURA, así que se equivoca todo lo que venga después, incluida la conclusión de que el otro se equivocó.**

- **El caso, medido:** Google Sheets exporta las celdas vacías **autocerradas** —`<c r="S2" s="2"/>`— y el patrón `<c([^>]*)>(.*?)</c>` **no las reconoce**: arranca en `S2`, consume `T2` entera buscando el primer `</c>`, y le adjudica a `S` el valor de `T`. **Los valores sangran una columna hacia atrás.** En una sola solapa había **4.645** celdas así.
- ⚠ **Lo caro no fue el número: fue la CONCLUSIÓN SOBRE EL OTRO.** `Tipo Campaña` parecía traer `GCBA`/`JM`/`Sin Tipo` —que son valores de la columna de ámbito— y de ahí salió una refutación entera, escrita con seguridad, contra una medición ajena **que estaba bien**. **Un lector roto no produce un dato dudoso: produce evidencia en contra de quien mide bien.**
- ⭐ **La pregunta concreta, y se hace al escribir el lector, no al primer número raro:** *¿qué formas puede tener la unidad que estoy partiendo, y mi patrón las cubre todas?* Un `<tag/>` y un `<tag>…</tag>` son la misma cosa para el formato y **dos cosas distintas para un regex**.
- ⭐⭐ **Y la salida no es leer con más cuidado: es un SEGUNDO LECTOR INDEPENDIENTE.** Acá fue `openpyxl` con `data_only=True`, cruzado **columna por columna sobre las 5.149 filas**. Cuesta diez líneas y es lo único que distingue *«mi lector está bien»* de *«mi lector es consistente consigo mismo»*. **Es el mismo argumento que las tres listas duplicadas a propósito de §2: el contra-qué tiene que ser independiente.**
- ⭐⭐ **Y la condición que hace que eso funcione, sin la cual el cruce da verde igual: los dos lectores tienen que FALLAR DISTINTO.** `openpyxl` y un regex propio sirven porque **uno resuelve la referencia de celda y el otro la posición** — son dos mecanismos, no dos implementaciones. **Dos regex escritos por la misma cabeza habrían coincidido en el error y la verificación habría pasado.** Independencia no es *«otro archivo»*: es *«otra forma de equivocarse»*.
- ⚠ **Y es la misma familia que la truncación del conector en modo markdown** —que devuelve 164 filas donde hay 221—: **ninguno de los dos falla, los dos devuelven algo publicable.** Cuando dos caminos de lectura del mismo archivo discrepan, **el hallazgo es el camino, no el dato**, y no se cita ninguno de los dos hasta resolverlo.
- ⛔ **El radio de alcance se mide y se escribe, aunque no se arregle en la misma corrida.** Acá eran **13 herramientas de `tools/`** compartiendo la clase rota. Arreglarlas movería números ya documentados sin que nadie los esté mirando, así que va como pendiente propio — pero **un radio no medido no se distingue de un radio de cero**.
...
docs/PREGUNTAS_AL_EQUIPO.md         qué se le preguntó al equipo y qué bloquea cada respuesta (vivo)
docs/CIERRE_POR_LAMINA.md           qué lámina está cerrada y qué le falta (vivo) — el ✅ es del usuario
docs/CAMINOS_universo_*.md          de qué filas sale cada marcador — congelado, fechado
...
| ¿Qué se le preguntó al equipo y sigue sin respuesta? | `docs/PREGUNTAS_AL_EQUIPO.md` — **vivo**, creado el 11/09/2026. **Una fila por pregunta: qué se pregunta · qué BLOQUEA · dónde vive la decisión.** ⛔ **No duplica texto, apunta**: el detalle sigue en el documento que esta tabla declara dueño. ⭐ Existe porque las preguntas estaban repartidas entre `PENDIENTES` —que ordena por hueco—, los `VALIDACION_*` congelados —que las guardan con su medición— y `CONFIG_INFORMES` —que las mezcla con decisiones ya tomadas—, y **ninguno contestaba la pregunta que importa al hablar con el equipo: qué preguntarle y qué se destraba con cada respuesta**. ⚠ Entra sólo lo que **contesta el equipo**: si lo puede contestar una medición, es un pendiente. ⚠ **Una respuesta parcial no cierra una fila** —`R-05` confirmó el criterio y sigue abierta por la señal—. Antes vivía como sección de `docs/PENDIENTES_consistencia.md`, que **deja de ser dueño de esta pregunta** | los dos |
...
| ¿Qué láminas están cerradas, y qué le falta a cada una de las que no? | `docs/CIERRE_POR_LAMINA.md` — **vivo, se edita**, una fila por lámina, **de `jm` y de `secco`** (`secco` entró el 10/09/2026, en su propia sección; hasta ahí era sólo de `jm`). Existe porque `D-38` cierra la fase cuando el usuario **mira un deck completo** y declara que lo que falta no es relevante, y eso pide un lugar donde decir *"esta lámina ya está, aunque le falten cosas"*. **Distinto de `VALIDACION_*`, que es una foto fechada organizada por bloque, y de `PENDIENTES`, que está organizado por hueco**: esto se organiza **por lámina**, que es la unidad en la que el usuario mira. ⛔ **El ✅ lo pone el usuario, nunca Code** — Code mueve a 🟡 con evidencia y ahí para | los dos; el ✅ **sólo** el usuario |
| ¿Por qué CAMINO sale el universo de filas de un marcador, y cuál cae a la solapa entera? | `docs/CAMINOS_universo_AAAA-MM-DD.md` — congelado, uno nuevo por relevamiento. **Existe porque ningún documento contestaba esto y la fila más parecida lo DESMIENTE**: `catalogo.js` declara que su columna `config` *«dice sólo que la configuración resuelve — no que el número salga de las filas correctas»*. ⚠ **No se puede autodocumentar como `OPERACIONES_`**: `datosDeMarcador_` es un `if/else` ordenado, no un mapa explícito, así que una rama nueva deja el documento incompleto **sin que nada falle**. ⚠ Distinto de `CATALOGO_tokens.md`, que dice **qué mide** un token: esto dice **de qué filas** | nadie edita; se crea uno nuevo |
...
| ¿Qué tokens de la plantilla NO tienen fila en `MARCADORES`? | `docs/CENSO_tokens_sin_fila_AAAA-MM-DD.md` — congelado, uno nuevo por corrida. **Existe porque hasta el 22/08 esa pregunta se contestaba mirando decks generados** —donde el `/////` del motor y el `-` que tipea el equipo son **indistinguibles** (`C-75`)— **o documentos que resultaron estar viejos** (`C-76`, `C-77`). Esto lo lee del registro contra la **plantilla viva**. ⚠ **«Sin fila» NO es «publica `FALTA`»**: es otra pregunta y necesita una corrida | nadie edita; se crea uno nuevo |
| ¿Qué trae el TEMARIO tal como llega, antes de que alguien lo adapte? | `docs/TEMARIOS_reales_AAAA-MM-DD.md` — congelado, uno nuevo por temario que aparezca. ⛔ **Existe porque hasta el 25/08 todo lo que el repo suponía sobre el temario salía de lo que quedó cargado en `REUNIONES`, que es el RESULTADO de una adaptación manual y no la entrada** — se razonó sobre el efecto y se lo tomó por la causa. ⚠ Distinto de la hoja `REUNIONES`, que dice qué hay cargado hoy | nadie edita; se crea uno nuevo |
...
| ¿De qué SOLAPA sale un dato, cuando hay varias candidatas? | `docs/FUENTE_*_AAAA-MM-DD.md` — congelado, uno nuevo por elección de fuente. **Existe porque «quién genera un dato» y «en qué solapa está cargado» son dos preguntas distintas**, y hasta el 25/08 el repo trataba la segunda como si se dedujera de la primera: *«sólo JM tiene POST»* se leyó como *«la solapa es la de JM»*. Mide **las candidatas todas**, no las que el prompt nombra, y declara contra qué deck se cruzó — o que **no se pudo cruzar**. ⚠ Distinto de `CENSO_solapas_*`, que dice **qué solapas hay**: esto dice **cuál sirve, y por qué las otras no** | nadie edita; se crea uno nuevo |
...
| ¿Qué universo de filas produce un CRITERIO DE CORTE, y cómo se compara contra la referencia externa? | `docs/MEDICION_*_AAAA-MM-DD.md` — congelado, uno nuevo por medición. **Existe porque el corte de un ámbito no es un número ni un camino: es un CRITERIO, y lo que hay que poder citar es qué filas selecciona cada candidato y cuánto se aleja del tablero.** ⚠ Distinto de `VALIDACION_*`, que mide un token contra un valor esperado, y de `CAMINOS_universo_*`, que dice por qué camino sale el universo de un marcador ya cableado: esto **compara criterios que todavía no se cablearon**. ⛔ **Declara la fecha de LECTURA de la referencia externa**, porque el tablero mueve un período ya cerrado (30/08: −1,03 % en GCBA en un día) | nadie edita; se crea uno nuevo |
...
| ¿Cómo entra alguien que no tiene compartidas las bases? | `docs/SEGURIDAD.md` — **vivo**, creado el 28/08/2026. Separa las **tres** preguntas que `D-15`, `D-16` y `D-18` mezclan —**identidad** (¿con qué cuenta entró?), **autorización** (¿qué puede pedir?) y **acceso al dato** (¿qué archivo puede abrir?)—, que es por qué la pieza 3 de `D-16` lleva un mes sin solución: se resuelven con mecanismos distintos. ⛔ **No elige, y no deroga nada**: las decisiones `D-NN` siguen viviendo en `PLAN.md` y superseder una es del usuario. ⚠ Distinto de `ENTORNO.local.md`, que dice **con qué cuenta** se entra: esto dice **con qué mecanismo se decide si puede** | los dos |

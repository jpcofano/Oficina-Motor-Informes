# Catálogo de tokens — generado desde `MARCADORES_2026-08-15.tsv`

> **Generado por `tools/catalogo.js` desde `docs/_snapshots/MARCADORES_2026-08-15.tsv`, `SOLAPAS_2026-08-15.tsv` y `MAPEO_2026-08-15.tsv` — los tres del 15/08/2026.** Es **evidencia fechada, no el estado de hoy**: para saber qué hay ahora se re-corre el script contra un snapshot nuevo. El dueño de la pregunta es el script, nunca este archivo.
...
**Contra el juego del 2026-08-15 da 78 de 78 que resuelven, y el motor publica DIEZ marcadores en error en una corrida real.** No es una contradicción: son dos preguntas distintas, y confundirlas es el modo de falla que este proyecto ya pagó.
...
⚠ **Este snapshot no tiene la columna `dimensiones`.** Nació el 15/08 con el piloto de `D-33`, después de esta toma, así que el catálogo **no puede mostrar el corte por dimensión de ningún marcador** — sólo el `filtro` de texto. No es que no haya cortes: es que esta foto es anterior a que se declararan.

**78 marcadores** · **78 con la configuración resuelta** · **0 sin resolver**.

## Familia `ecv` — 16 token(s)

| token | informe | medida (`campo_logico`) | base / solapa | operación | filtro | formato | config |
|---|---|---|---|---|---|---|---|
...
| token | informe | medida (`campo_logico`) | base / solapa | operación | filtro | formato | config |
|---|---|---|---|---|---|---|---|
| **`gcba_mail_envios`** | `jm` | `mail_enviados` | `digital/Directa Mail` | `SUMA` | `mail_remitente!=jorge.macri@buenosaires.gob.ar` | `miles` | resuelve |
| **`gcba_mail_entregados`** | `jm` | `mail_entregados` | `digital/Directa Mail` | `SUMA` | `mail_remitente!=jorge.macri@buenosaires.gob.ar` | `miles` | resuelve |
| **`gcba_mail_aperturas`** | `jm` | `mail_aperturas` | `digital/Directa Mail` | `SUMA` | `mail_remitente!=jorge.macri@buenosaires.gob.ar` | `miles` | resuelve |
| **`gcba_mail_or`** | `jm` | `mail_aperturas/mail_entregados` | `digital/Directa Mail` | `PCT` | `mail_remitente!=jorge.macri@buenosaires.gob.ar` | `porcentaje_sin_signo` | resuelve |
...
| **`gcba_imp_total`** | `jm` | `Impresiones` | `looker/DIGITAL` | `SUMA` | `nombre_campaña!~=JM && estado=Activa` | `miles` | resuelve |
...
| token | informe | medida (`campo_logico`) | base / solapa | operación | filtro | formato | config |
|---|---|---|---|---|---|---|---|
| **`imp_total`** | `jm` | `Impresiones` | `looker/DIGITAL` | `SUMA` | `nombre_campaña~=JM && estado=Activa` | `miles` | resuelve |
| **`imp_meta`** | `jm` | `Impresiones` | `looker/DIGITAL` | `SUMA` | `nombre_campaña~=JM && estado=Activa && Plataforma=Meta` | `miles` | resuelve |
| **`imp_google`** | `jm` | `Impresiones` | `looker/DIGITAL` | `SUMA` | `nombre_campaña~=JM && estado=Activa && Plataforma=Google ads` | `miles` | resuelve |
| **`imp_prog`** | `jm` | `Impresiones` | `looker/DIGITAL` | `SUMA` | `nombre_campaña~=JM && estado=Activa && Plataforma!=Meta && Plataforma!=Google ads` | `miles` | resuelve |
| **`gcba_imp_meta`** | `jm` | `Impresiones` | `looker/DIGITAL` | `SUMA` | `nombre_campaña!~=JM && estado=Activa && Plataforma=Meta` | `miles` | resuelve |
| **`gcba_imp_google`** | `jm` | `Impresiones` | `looker/DIGITAL` | `SUMA` | `nombre_campaña!~=JM && estado=Activa && Plataforma=Google ads` | `miles` | resuelve |
| **`gcba_imp_prog`** | `jm` | `Impresiones` | `looker/DIGITAL` | `SUMA` | `nombre_campaña!~=JM && estado=Activa && Plataforma!=Meta && Plataforma!=Google ads` | `miles` | resuelve |
...
| token | informe | medida (`campo_logico`) | base / solapa | operación | filtro | formato | config |
|---|---|---|---|---|---|---|---|
| **`mail_envios`** | `jm` | `mail_enviados` | `digital/Directa Mail` | `SUMA` | `mail_remitente=jorge.macri@buenosaires.gob.ar` | `miles` | resuelve |
| **`mail_entregados`** | `jm` | `mail_entregados` | `digital/Directa Mail` | `SUMA` | `mail_remitente=jorge.macri@buenosaires.gob.ar` | `miles` | resuelve |
| **`mail_aperturas`** | `jm` | `mail_aperturas` | `digital/Directa Mail` | `SUMA` | `mail_remitente=jorge.macri@buenosaires.gob.ar` | `miles` | resuelve |
| **`mail_or`** | `jm` | `mail_aperturas/mail_entregados` | `digital/Directa Mail` | `PCT` | `mail_remitente=jorge.macri@buenosaires.gob.ar` | `porcentaje_sin_signo` | resuelve |

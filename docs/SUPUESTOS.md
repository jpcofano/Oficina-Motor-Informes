...
| ~~**S-06**~~ *(25/08/2026 — **DEROGADO el mismo día**, ver abajo)* | ~~**`L-036` es por PLATAFORMA, con las cuatro ranuras en el orden `TOTAL · Meta · Google · Programmatic`**~~ | ⭐ el **grano** está decidido por el usuario (25/08) con tres evidencias: el `Formato` difiere por plataforma para la misma campaña, el deck del equipo publica el POST desglosado, y los cuatro bloques de `Agenda JM \| Post` **cierran al dígito** con la suma del desglose por plataforma. ⛔ **Lo que NO está medido es el ORDEN de las ranuras**: el orden de las *columnas* de la derivada es TOTAL·Meta·Google·Prog, pero eso no prueba el orden de las *filas* de la tabla, y el deck del equipo publica el POST **sin fila de TOTAL** (Meta·Google·Programmatic a secas) | ⛔ **la lámina publica un número correcto en la fila equivocada**, que es el modo de falla más caro del repo y el único que no avisa. El síntoma es que `post_impresiones1` traiga el valor de Meta y no el total — **plausible en las dos lecturas** | se reordenan las cuatro ranuras en `cablearTablaPostPorPlataforma_`. ⚠ **Mientras el supuesto no se confirme mirando la lámina pintada, el cableado NO se aplica** (decisión del 25/08): están escritos el `MAPEO`, la sección y la reversión, y falta el paso que escribe en `MARCADORES` |

## Derogados

- **`S-06` — "`L-036` es por PLATAFORMA"** — **derogado el 25/08/2026, el mismo día que se
  registró.** Decisión del usuario: **el desglose por plataforma es del «1 a 1» (`L-053`), no de
  esta lámina.** `L-036` es **por REUNIÓN, una fila con el TOTAL de esa reunión** — el diseño
  original, que nunca estuvo mal.

  ⭐ **Lo medido no se descarta: encaja.** `col12` de `Agenda JM | Post` **es el TOTAL** —41.204
  para Retiro, al dígito contra la suma de sus filas POST del desglose— y los otros tres bloques son
  las plataformas, **que esta lámina no usa**.

  ⭐⭐ **Y el supuesto se cierra sin quedar abierto, que es lo importante:** el riesgo que declaraba
  era *«un número correcto en la fila equivocada»*, y **con la fila = el encuentro deja de existir**.
  `Habitantes` y `Alcance` son **del encuentro** y la fila **también**, así que ya no hay dos granos
  que confundir. Los tres faltantes encadenados —orden de ranuras, qué columna va en qué fila, y el
  grano de Habitantes/Alcance— **se disuelven los tres juntos**.

  ⚠ **Lo único que sobrevive del rodeo como pendiente** es el `Formato` desde `Nomenclatura`, que
  era un bloqueo independiente del grano.

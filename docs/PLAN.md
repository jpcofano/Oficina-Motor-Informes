...
| `[x]` **15** | ✅ **CERRADO el 08/09** — ~~Remitente sin normalizar (`A5`)~~ | la decisión existe y **vive en `CONFIG_INFORMES` §4.9** (`D1`): la columna «Envío» publica el **ámbito**, no el mail, con la advertencia de que es la misma condición que `DIMENSIONES_` aplica y que se desincronizan en silencio. ⭐ **Y el remitente NO estaba sin normalizar: lo está, en otra solapa** — `acumulado \| Mail`, la misma base que Call Center. Lo que falta es el **alta** de esa solapa, y eso es el ítem **42** |

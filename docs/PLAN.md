...
| `[ ]` **34** | ⛔ **`emin_lista` publica `-` en el deck** — el defecto sigue ABIERTO | ⛔⛔ **La hipótesis murió DOS veces** (04/09) y **el ítem no se tacha**: ① `diagTokensEmin()` midió que `{{emin_lista}}` **sí está** en slide 14 con los otros nueve; ② y la de la caja también, porque **`textoFaltante_` devuelve `'-'` para `sin_datos`** (`Generador.gs:1935`) ⇒ **ese guión es del motor**. ⭐⭐ **Lo que queda:** el token existe, se resolvió y **volvió vacío**. `LISTA_TEXTO` devuelve `''` si le falta `ctx.plantilla` o si no recibe filas, y `Generador.gs:1391` baja un texto vacío a `sin_datos`. ⇒ **Arreglo de la operación o del cableado, NO de la plantilla** |

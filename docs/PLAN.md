...
| **Los estados `-` y `---`** — ✅ **DECIDIDO Y DIFERIDO** (usuario, 16/08/2026). **No es un pendiente: está resuelto.** **Se queda `«FALTA:token»`** mientras `S-05` valga —el deck lo lee sólo quien lo desarrolla— porque el `«FALTA:»` crudo **dice más**. `---` se implementa **el día que haya lector externo**, y ése es el disparador, no una fecha. **Esto está anotado como decidido a propósito:** un pendiente que en realidad está resuelto hace que alguien lo levante de nuevo y lo vuelva a discutir | **su reversión ya vive en la columna de `S-05`**, que es donde corresponde: el día que el supuesto caiga, esto se revisa con él. **Salvaguarda que sigue vigente:** el reporte de corrida tiene que **seguir distinguiendo *"no calculable"* de *"falló el cableado"*** aunque el deck deje de hacerlo | usuario — **ya decidió** |

...
| | | dónde vive el detalle |
|---|---|---|
| `[ ]` **37** | ⭐⭐ **El ALTA de la base + el cableado de los TRES tokens ya validados** — `cc_base`, `cc_contactados`, `cc_contact_pct` | **Listo para escribirse.** El recorte está validado en las **dos** ventanas y por **tres magnitudes independientes** (`V-126`…`V-130`), y `C-78` ya había medido que el mecanismo existe —fecha propia + `ambito` sobre columna propia, la forma de `mail_entregados`—: lo que faltaba era **una solapa con la columna**, y `Call Center - Métricas` trae `Fecha` (K) y `Remitente` (Q). Escribe `BASES`, `SOLAPAS` (`ventana_ref` **vacío**: tiene fecha propia), `MAPEO` y `MARCADORES`. ⛔ **Su gate es `C-115`**, y es una línea: **verificar que el marcador que se cree lea la base nueva y no `looker/CC`** — el cruce caso→marcador **no scopea por base**. ⚠ Y el ámbito va **positivo por los dos lados** (`= JM` / `= GCBA`, `C-108`), que **diverge a propósito** de `ivr_vocero != JM` |
...
| `[ ]` **39** | ⚠ **`probar-guiones-grupos.js` en rojo** — `CASOS_POR_MARCADOR_` no conoce el CSV del 08/09 | Lo dice el propio banco: *«la constante dice 5 CSV y en disco hay 6»*. ⛔ **NO se regeneró a propósito**, y hoy es seguro no hacerlo: **cero marcadores `cc_*` vivos** (medido sobre `MARCADORES_2026-08-31.tsv`), así que el cruce produce entradas **inertes**. ⭐ **Deja de ser inerte con el ítem 37**: el día que el alta cree marcadores con esos nombres, `D-60` les levanta el `_revisar`. ⇒ **Se regenera JUNTO con el 37 y después de su gate**, nunca antes |

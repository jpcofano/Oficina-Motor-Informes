...
| | | |
|---|---|---|
| `[ ]` **9** | ⛔ `camp_titulo` en `L-016`/`L-023`: título de **otra** campaña | ⚠ **NO ARRANCÓ el 03/09 — dos premisas sin verificar.** (a) **No se encontró el prompt**: los que mencionan `camp_titulo` son de agosto y están ejecutados. (b) **`L-023` figura `escondida = sí`** en `LAMINAS`, así que o no se ve en el deck o el registro está viejo (`P2` de las columnas-foto). ⭐ Lo medido: `ULTIMO` sobre `digital/Seguimiento digital · sd_campana_cuentas`, **sin `filtro` ni `dimensiones`** ⇒ depende **enteramente** de `opciones.id_cuenta`, y `PENDIENTES` ya registra que resuelve con `@ultimo_ambiguo` en otras láminas |

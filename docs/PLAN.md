...
| `[ ]` **34** | ⛔ **`emin_lista` no salió en el deck del 03/09** | ⭐⭐ El deck trae `-` con **UN** guión y `_revisar` envuelve en **DOS** ⇒ **ese guión no es del motor** (`C-75`). Hipótesis: `{{emin_lista}}` **no existe en la plantilla** — el modo de falla **mudo**: resuelve, no encuentra dónde pintarse y **no entra a `FALTANTES`**. ⭐ Se mide con **`diagTokensEmin()`**, con control positivo. ⚠ Y la caja **«Alcance» está vacía sin guiones**: espera un token que nadie cabló |
...
| `[ ]` **33** | ⛔ **Ítem 9 — por qué `L-016` no se duplica** | ⭐ Hipótesis del ancla **MUERTA**: `verLaminas()` dio `filas_sin_ancla = 0`. Quedan **dos**: `seccion_id` en la hoja viva, y la **guarda de contigüidad** — ⚠ **ésta sale del reporte de la próxima corrida**, no de la plantilla |

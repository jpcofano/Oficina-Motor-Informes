...
| `[ ]` **36** | ⛔⛔ **El cruce marcador ↔ caso corre en UNA sola dirección** | ⭐⭐ **A seis `m2_*` se les puso `_revisar` DESPUÉS de que `V-124` los validara** (02/09), y **nada avisó** — lo detectó el usuario **leyendo el deck**. Se revisa **al levantar** la marca, nunca **al ponerla**. ⇒ Falta el cruce **al revés**: *caso `exacto` cuyo marcador sigue con `_revisar`*. ⚠ El costo no es cosmético: **un dato validado entre guiones le enseña al lector que los guiones no significan nada** |

...
| `[x]` **34** | ⛔ **`emin_lista` publica `-`** — ✅ **ARREGLADO el 05/09** | ⛔⛔ **`ctx.plantilla` nunca llegaba.** Se asignaba dentro del `if` de `['FILA','FILA_TEXTO','GRUPO_TEXTO']` y **`LISTA_TEXTO` no está en esa lista** — la guarda `esPlantilla` sí lo incluye, pero estaba **anidada dentro del `if` que ya lo dejó afuera**. ⇒ `«FALTA:@plantilla_sin_resolver»` → `sin_datos` → `-`. ⭐⭐ **El arreglo NO fue agregar el nombre a la lista**, que es lo que decía este ítem: fue **sacar la guarda del `if`**, porque `LISTA_TEXTO` usa `separador` como **cadena que une** y no como campo de orden — medido, con el camino (1) `buscarMapeo` recibiría `"—"` como nombre de campo. Hoy no rompería **sólo porque `emin_lista` declara `separador` vacío**, y esa celda está diseñada para llenarse. ⭐ **El precedente es del propio repo, 26/08**: `ctx.separador` salió de ese mismo `if` por la misma razón, con el comentario *«va incondicional y no detrás de otra lista de nombres»*. **La lista de nombres ya existía —`esPlantilla`—; lo que faltaba era usarla.** · banco `tools/probar-ctx-plantilla.js`, con negativo que reconstruye la anidación vieja · ⚠ **falta la corrida**: el banco prueba que el `ctx` se arma, no qué publica · ⭐⭐ **06/09 — LA CORRIDA EXISTE, y dice dos cosas opuestas.** `emin_lista` publica **los siete renglones** en `L-012` de `secco` con `emin_encuentros` en `-7-` ⇒ **el arreglo funciona end to end** (`C-100`). ⛔⛔ **Y ese verde es falso sobre el universo:** la lista trae `04/09` y `08/09`, fuera del encabezado `28/08 – 03/09`, y **son otras siete filas** — sobra Quirós, falta Sabor (`C-101`). ⚠ **El control de conteo dio verde sobre la lista equivocada**, porque las dos diferencias se cancelan en el total: ningún control que compare cantidades lo puede ver. ⇒ **Son dos cosas distintas** — el `ctx.plantilla` está bien, **la ventana de la solapa no** —, y lo destraba el desplazamiento por solapa del 06/09, con la celda **vacía hasta que el usuario cargue el valor** |
| `[x]` **35** | ⭐ **El censo de no exclusivos tiene que CLASIFICAR, no contar** | ✅ **HECHO el 04/09**, con las tres decisiones del usuario: **reemplaza** al censo viejo, **la escondida se IGNORA** —un token en dos láminas, una escondida, cuenta sólo por la visible— y es **un botón**, no algo que corra en cada corrida. ⭐ Reparte en **sanos** (salen de la lista) y **sospechosos** (el número accionable). ⚠ **Y declara lo que NO puede medir:** una lámina con ancla que aun así no se expande —contigüidad, corte, o no elegida— **sólo la nombra el reporte de la corrida**; el testigo es `L-016` |
...
| `[x]` **31** | ⭐ **FRONT — `D-57`: el botón único necesita progreso SIN cambiar de pestaña** | ✅ **HECHO el 04/09, junto con `D-57`** — era su gate y se hizo **en el mismo cambio**, que es lo que permitió levantarlo sin ignorarlo. Sondeo cada 15 s en la misma pantalla, y **vista final propia** —no la rama `listo`, que habría pintado ceros |
...
| `[~]` **33** | ⛔ **Ítem 9 — por qué `L-016` no se duplica** | ⭐⭐ **CAUSA RAÍZ ENCONTRADA el 04/09 — y arreglada.** Las dos candidatas anteriores estaban descartadas; la tercera es un **off-by-one estructural**: `indiceDeLaminasPorAncla_` se calcula **una vez antes de duplicar** —correcto, mata la N²— pero `getSlides()` se relee **por sección**, ya movido ⇒ **las secciones posteriores a la primera indexan el deck nuevo con posiciones viejas**. Evidencia: `LAMINAS` declara `L-016`…`L-023` y el deck duplicó `L-017`…`L-024`, donde **`L-024` es `analisis_datos`**. ⭐ Arreglado **por `objectId`** —el ancla no sirve: las copias lo heredan— más la guarda de lámina perdida. Banco `probar-bloque-por-objectid.js`. ⛔ **Falta la corrida que lo confirme** |
| `[x]` **2** | `emin_lista` — operación con **plantilla sobre todas las filas**. `LISTA_CRUDA` toma un campo, deduplica y no acepta plantilla | ✅ **HECHO el 03/09** — `LISTA_TEXTO`, la decimocuarta operación, registrada en `OPERACIONES_` y ruteada en el despachador. Plantilla sobre TODAS las filas, unidas por `separador`, **sin deduplicar** — que es la decisión que la define: dos encuentros distintos pueden dar el mismo texto y siguen siendo dos. Banco `probar-lista-texto.js` |
| `[x]` **3** | El condicional del barrio **DECLARATIVO**, no un `if` con literal — la propuesta usa `valor_fijo` (condición) + una plantilla alternativa | ⛔ **CANCELADO por decisión del usuario, 04/09** — la lista es `funcionario + barrio + fecha`, **sin condicional**. ⭐ El mecanismo `{campo=VALOR?alterno}` **queda en el motor y funciona** (banco en verde); lo que se cae es su uso en `emin_lista`, porque **con el barrio siempre presente las tres filas del ciclo ya se distinguen**. ⚠ La justificación vieja queda **vencida, no borrada** |
...
| `[~]` **7** | ⭐ **Los 45 compartidos sin fila** — sirven a los **dos** informes | ⭐ **Enumerador escrito el 04/09**: `listarCompartidosSinFila()`, sólo lectura, sobre las **dos plantillas vivas**. ⚠ **El número circulaba sin la lista** y además en dos versiones —`PLAN.md` decía **56**, la cola **45**—. ⛔ **Y aplica la regla del usuario del 04/09**: los `*_bench_*` y lo que vive **sólo en láminas escondidas en las DOS** se **congelan**; los **mixtos** —escondido en una, visible en la otra— **NO**, porque el token es compartido y el lado que lo pinta lo necesita. ⇒ **Falta correrlo** para saber el lote real |
...
#### CALL CENTER — la base nueva (08/09/2026)

> ⭐ **Puntero, no copia.** Los números y su evidencia viven en
> `docs/casos_validacion_2026-09-08.csv` y en los dos `MEDICION_*_2026-09-08.md`. Acá está **el
> orden**.

| | | dónde vive el detalle |
|---|---|---|
| `[x]` **37** | ✅ **CERRADO el 08/09** — ~~El ALTA de la base + el cableado de los TRES tokens ya validados~~ — **`L-031` publica `6.011 · 1.878 · 31,2 %`** en el deck de `julio_24_30` (corrida del 08/09 21:23), que son exactamente sus testigos `V-126`, `V-128` y `V-129`. ⛔ **Y destapó el ítem 41.** El texto original decía:** — `cc_base`, `cc_contactados`, `cc_contact_pct` | **Listo para escribirse.** El recorte está validado en las **dos** ventanas y por **tres magnitudes independientes** (`V-126`…`V-130`), y `C-78` ya había medido que el mecanismo existe —fecha propia + `ambito` sobre columna propia, la forma de `mail_entregados`—: lo que faltaba era **una solapa con la columna**, y `Call Center - Métricas` trae `Fecha` (K) y `Remitente` (Q). Escribe `BASES`, `SOLAPAS` (`ventana_ref` **vacío**: tiene fecha propia), `MAPEO` y `MARCADORES`. ⛔ **Su gate es `C-115`**, y es una línea: **verificar que el marcador que se cree lea la base nueva y no `looker/CC`** — el cruce caso→marcador **no scopea por base**. ⚠ Y el ámbito va **positivo por los dos lados** (`= JM` / `= GCBA`, `C-108`), que **diverge a propósito** de `ivr_vocero != JM` |
| `[ ]` **38** | ⛔ **`cc_campanias` — 4 candidatas y ninguna evidencia que las separe** | `C-112`. De nueve candidatas medidas, aciertan las dos ventanas la **2** (`Tipo de llamado` distintos), la **3** (`Campaña` distintos), la **7** (`Tipo` distintos) y la **8** (prefijo de `Campaña`). ⚠ **Y agosto no discrimina: las nueve dan 3** — todo el peso está en julio. ⭐ El desglose las parte en **dos familias que etiquetan la misma fila con palabras distintas** (`C-114`), y hay un **argumento estructural** para la tipada: `V-91`/`S-01` ya filtran por `Tipo de llamado IN (Convocatoria; IVR convocatoria)` y esas cadenas están textuales en la columna `R`. ⛔ **No se desempata con estas dos ventanas.** ⇒ **decisión del usuario**, o una tercera ventana donde difieran. **Bloquea SÓLO este token**, no los otros tres |
...
| `[ ]` **42** | ⛔ **La columna «Envío» de `L-047` no tiene camino declarativo** | `D1` (08/09) decidió que publique el **ámbito**, `JM`/`GCBA`. Medido con dos lectores sobre el fixture del 30/08: de las **25 columnas** de `digital/Directa Mail`, **cero** traen ese valor, y la única que particiona es `Mail remitente` —el mail crudo—. ⛔ **`mail_area` NO discrimina**: 24 áreas, JM en tres de ellas, y *Jefatura de Gobierno* no es exclusiva suya. ⇒ **lo destraba `C-01`** (una columna de ámbito en la base), no un alta de `MAPEO`. El gate `G1` de `aplicarTanda20260908()` lo deja listo: las cinco filas están escritas con `campo_logico` **vacío**, y el día que la columna exista es apretar un botón. `CONFIG_INFORMES` §4.9 |

#### PUBLICA MAL HOY

| | | |
|---|---|---|
| `[ ]` **9** | ⛔ `camp_titulo` en `L-016`/`L-023`: título de **otra** campaña | ⭐ **Parte 0 hecha el 03/09 — mecanismo EXPLICADO, causa raíz NO cerrada** (`PENDIENTES` `P0`). `camp_titulo` **no es exclusivo de ninguna lámina** ⇒ `pintarTokensFijosDeLamina_` usa `presentacion.replaceAllText`, que **pinta el deck entero, escondidas incluidas**, con **un solo valor**. ⭐⭐ Eso explica que `L-023` publique con las llaves de sus `camp_resp_*` todavía crudas, y que sus dos copias digan lo mismo. ✅ El precedente `L-040`/`rol=equipo` **NO aplica**: `L-016` es `rol = motor`. ⛔ **Falta**: si `L-016` se expandió, y cuántas campañas tenía el temario — las dos exigen el **reporte de la corrida** y `CAMPANAS` **vivo** (el snapshot del 31/08 tiene 2 filas y las dos son de `jm`) · ⭐⭐ **05/09 — y hay una variable que la nota no tenía: CUÁL campaña sale DEPENDE DE A QUÉ HORA SE CORRA.** El 04/09 el deck de las **10:45** publicó `Operativo Muro | 25/8` y el de las **11:42** `Fin de las mafias de los celulares robados` — **misma configuración, dos campañas**. ⇒ Es un insumo del diagnóstico, no otro defecto: **cualquier comparación de `camp_titulo` entre dos decks tiene que declarar la hora**, o mide la selección en vez del pintado. ⚠ Y explica por qué dos lecturas del mismo síntoma pudieron discrepar sin que ninguna estuviera mal |
| `[x]` **10** | ⭐ **Un caso `V-` para `frecuencia` de esa campaña** | ✅ **CERRADO el 05/09 por `C-95`, y no como se esperaba.** La pregunta era *«¿32,7 es correcto?»* contra el deck; la respuesta es que **no hay defecto que medir**: `R-34` fija que `camp_alcance` publica el alcance de Meta —única columna disponible— y que **el equipo no actualiza la base**, así que `camp_frecuencia` **hereda entero** ese denominador. `28.540.835/872.827 = 32,7` contra `28.540.942/1.271.754 = 22,4`: las impresiones coinciden casi al dígito y **toda la brecha viene del denominador**. ⇒ **Un derivado no tiene vida propia respecto de sus insumos** — mismo criterio que frenó los dos `*_vtr` en el `_7`. ⚠ **El matiz que quedaba «no medido» queda sin objeto**, no medido: no hay agregado que discutir sobre una fila que se explica entera |
| `[x]` **11** | IVR: `ambito=gcba` da **63** donde deberían ser **8** · ⛔ **¿cableado o `C-01`?** | ✅ **CERRADO el 04/09** — `censarIvrEnPlantillaJm()` con control positivo: **los tres están en la lámina 3**, y `gcba_cc_base` también. ⇒ ⛔ **La hipótesis (1) del `2026-08-31_2` —*«la plantilla no usa el token»*— queda DESCARTADA**: sigue siendo un **cableado**, no `C-01`. ⭐ Confirmado aparte por el deck del 04/09, donde `ivr_llamados` y `gcba_ivr_llamados` **ya publican distinto**. ⚠ `TOKENS.md` §178 quedó **viejo** —era cierto el 16/08 y la plantilla cambió el 29/08 |
...
| `[~]` **15** | Remitente sin normalizar (`A5`) | ⭐ **La decisión EXISTE desde el 08/09** y está escrita en `CONFIG_INFORMES` §4.9: la columna «Envío» publica el **ámbito**, no el mail (`D1`). ⛔ **Lo que falta ya no es decidir, es poder escribirlo** — ninguna columna de la fuente trae `JM`/`GCBA`. Pasa a ser el ítem **42** |
...
| `[x]` **21** | Los **2 bancos en rojo**, defectos reales | ✅ **CERRADO el 03/09 — es el ítem 30**, que lo hizo. Duplicado: los dos bancos son los mismos. ⭐ Se dieron vuelta **con la exigencia mayor** —identidad de complementariedad en vez de constante— y hoy **los 92 pasan** |

...
**Última actualización:** 2026-08-25 — corrida nocturna `2026-08-24_2`, **las cinco partes hechas**.
El conteo de faltantes pasa a ser confiable: `FALTANTES` guarda `lamina_id` y el conteo dice **tres
números**. Más la medición de las seis solapas de `L-036`, pedida a mitad de turno.

⛔ **Nada de esto está en Apps Script todavía.** Ver *«Lo que te espera»*, abajo.

---

## ⛔ Lo que te espera, en orden — sin esto la corrida nocturna no existe

| # | qué | por qué |
|---|---|---|
| 1 | **`clasp push`** | nada de lo de anoche está en el proyecto. Un push que corrió antes del cambio es indistinguible de uno que no corrió |
| 2 | **`instalar()`** | crea `LAMINAS.alcance` y `LAMINAS.tokens_equipo` por `COLUMNAS_DELTA_`. **No siembra**: eso es *Aplicar configuración* |
| 3 | **`declararAlcanceDeLaminas()`** | puebla las dos columnas. **Después de 2**, o falla nombrando la columna que falta |
| 4 | **Generar `jm`** | es lo único que llena `FALTANTES` con `lamina_id`. Hasta entonces el corte por lámina se ve vacío **con su motivo** |
| 5 | ⚠ **`reserva_cierre_seg` a 60, A MANO** | `CONFIG` **sólo siembra lo ausente**. Sigue pendiente de antes |
...
## ⏸ `L-036` — la fuente NO está mal elegida, y el `id_cuenta` sigue siendo la candidata

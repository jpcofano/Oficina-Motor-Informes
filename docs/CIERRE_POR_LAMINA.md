...
| **7** · Campañas · pie | `L-036` | ⛔ 🕳 | ⛔⛔ **CORREGIDO 22/08 (tarde): son `camp1-4`, NO 32 `post_`.** La entrada anterior de esta fila decía lo contrario y era falsa. **`jm` no tiene ningún token `post_`** — lo dicen tres fuentes, una medida **sobre la plantilla viva**: `TOKENS.md` §2.1, el `P2` de `PENDIENTES` (`slidesModeloDe_(['post_'])` = **lista vacía**, 06/08) y el espejo (`camp1..camp4` en la primera columna, cero `post_`). **`Auditoria.gs` está mal redactado** — confundió `seccion_id = comunicaciones_post` con una familia de tokens. ⭐ **`camp1-4` es un caso de `ELEMENTO`**: el nombre de la i-ésima campaña. ⚠ **Sin resolver: qué hay en las otras 7 columnas** — eso necesita la plantilla viva (`C-76`) | 22/08 |

...
| **2** · Resumen Ejecutivo JM | `L-031` | ⛔ **inestable por CAMBIO** 🕳 🌐 | 21 tokens. ⭐⭐ **08/09: TRES de los cuatro `cc_*` PUBLICAN** — `6.011 · 1.878 · 31,2 %`, sobre la base nueva `acumulado` (`V-126`, `V-128`, `V-129`); `cc_campanias` sigue en `/////` (`C-112`, cuatro candidatas). ⛔ **Y `L-032` sigue en `/////`**: sus tres `gcba_cc_*` los escribe `aplicarTanda20260908()`, sin correr todavía. ⚠ El párrafo de abajo es de antes de esa medición: **Los cuatro `cc_*`: `MAPEO` escrito, cableado FRENADO** — la columna y la operación están validadas, **qué cuentas entran no**. ⭐ **25/08: la regla YA ESTÁ DECIDIDA** — `JDGAG` + pertenencia + `duración ≤ 30 d`, publicada en `_revisar`, elegida por **modo de falla** y no por acierto (`CONFIG_INFORMES` §4.7). ⛔ **Pero el freno cambió de motivo, y eso es el resultado del 25/08: ya no falta decidir, falta poder ESCRIBIRLO.** `duración ≤ 30 d` **no es expresable** en `MARCADORES` —los filtros comparan el valor de una celda, no una resta entre dos fechas— y el único tope por duración que existe (`R-30`) es **global** · `contenidos_total` publica `1` → **pregunta al equipo** · `frecuencia` no publica → sin medir · los 8 `imp_*` y los 3 «N envíos» están 🟡, abajo | 22/08 |
...
| **5** · ECV: alcance semanal | `L-034` | ⛔ **inestable por ALTA** 🕳 | ⭐⭐ **El 🌐 SE SACÓ el 25/08**: los 16 `ecv_*` con fila leen `rdv/RVD JM-CM - ES` y toman el universo del **temario** desde el `_25` (22/08) — medido uno por uno contra el snapshot de `MARCADORES`. ⚠ **Queda como CONDICIÓN, no como estado:** si el temario no trae filas, caen al universo ancho **en silencio** (ver la sección 🌐). ⭐ **`ecv_inscriptos` = 2.333 y `ecv_encuentros` = 4 REPRODUJERON** contra `V-71`. ⚠ **Pero `rdv` es inestable por ALTA (`R-31`), así que eso no se puede EXIGIR en cada corrida** — el control posible es **de dirección: sube o queda, nunca baja; si baja es bug**. Falta: `ecv_asistentes` = 485 sin validar · ⛔⛔ los 3 `cc_*` **NO se pintaron: salen `/////` con fila escrita y publicando en `L-031`** (08/09), y con ellos `Impresiones`, `Mails entregados`, `Aperturas (OR)` y `Atendidos`, que **sí publicaban el 22/08**. Es la resolución por lámina de `D-47` devolviendo nada — ver la corrección de abajo · `ecv_fecha` y `ecv_barrio1-3` **sin fila en `MARCADORES`** | 22/08 |
...
| **18** · Desagregados · Mail | `L-047` | ⛔ 🕳 | 5 envíos + GLOBAL, 9 columnas. ⭐ **24/08: la fila GLOBAL quedó COMPLETA** — se cablearon los tres que faltaban (`camp_enviados`, `camp_or`, `camp_mail_clics`) junto a los seis del `_19_1`. **Los 40 de envío ya tienen fila** desde el 23/08 (`FILA`, `X-35` cerrado). ⚠ Las celdas están **combinadas**, no vacías, y por eso son 40 y no 45. ✅ **08/09: la corrida existió y la lámina publica.** Los tres envíos poblados cierran la identidad interna —121.789 + 145.744 + 176.870 = **444.403**, el GLOBAL—. ⛔ **Falta la columna Envío**: `camp_env1_rem` publica el **mail crudo** y las otras cuatro `/////`; `D1` (08/09) decidió que diga `JM`/`GCBA` y **ninguna de las 25 columnas de la fuente trae ese valor** — es `C-01`, no un alta de `MAPEO` (`CONFIG_INFORMES` §4.9). ⭐ **`camp_env4_fecha` ya tiene sus llaves** y su fila la escribe `aplicarTanda20260908()`. ⭐ **El control primario no depende del deck del equipo**: `% OR = aperturas/entregados` y `% CTOR = clics/aperturas` son derivables de las otras celdas de la fila, y el GLOBAL tiene que ser la **suma** de los cinco envíos —ese cruza fuentes (`looker` contra `digital`) y por eso puede fallar legítimamente | 24/08 |
...
> ⛔⛔ **DESMENTIDO el 08/09/2026 por una corrida, y el hallazgo es mejor que la advertencia.**
> *«Cablear uno pinta las dos»* **es falso**: las tres se cablearon
> (`cablearCallCenterAcumulado()`), `L-031` publica **6.011 · 1.878 · 31,2 %** y **`L-034` sale
> `/////`** en el mismo deck (`julio_24_30`, corrida del 08/09 21:23).
>
> ⭐ **Y no son sólo los tres `cc_*`.** Cruzado contra el deck testigo del 22/08 (`sha256`
> `cd6f0050…`), en `L-034` **cuatro casilleros que PUBLICABAN pasaron a `/////`**: `Impresiones`
> (28.988.260), `Mails entregados` (538.276), `Aperturas (OR)` (210.707 (39.1 %)) y `Atendidos`,
> que era **`-`**.
>
> ⭐⭐ **`-` → `/////` es la prueba limpia:** `-` es *«se preguntó y no había dato»* y `/////` es
> *«no hay fila, o no se resolvió»* (`textoFaltante_`). **El dato no puede mover un token de un
> símbolo al otro.** Y no fue un corte por presupuesto: un tramo no alcanzado deja el token
> **crudo**, y el mismo deck lo muestra en las láminas 21, 22 y 24 (`{{m2_clics_a}}`, `{{rrss_*}}`).
>
> ⇒ **La advertencia de arriba apuntaba al riesgo correcto y al mecanismo equivocado.** El
> problema no es que las dos láminas compartan universo: es que **la resolución por lámina de
> `D-47` devuelve nada en `L-034`**. Ver la §⏸ del 27/08, más abajo — que declaró *«lo esperable en
> `L-034` no es otro número: es SIN DATO»* y **salió `/////`**.
>
> ⛔ **Por eso el `2026-09-08_7` NO escribió ninguna fila de `L-034`:** un token que existe y no
> resuelve no se arregla con una fila nueva; se taparía el síntoma. Medición completa en
...
| `L-012` | ministros | 🟡 | 3 valores · 9 entre guiones | ⭐⭐ **control primario: las TRES identidades internas, 3 de 3** (`V-140`…`V-142`) — reemplazan al cruce contra el equipo, cuyo bloque está **congelado** desde el 04/09. Más el bloque de **mail exacto** (`V-138`) y **11 de 11** encuentros (`V-137`). ⛔ Falta: la caja **Alcance** queda **SIN FUENTE** —no hay alcance deduplicado por ventana en ninguna solapa de `reuniones`, medido el 10/09, misma forma que `C-85`— y el bloque digital sigue con `C-123`/`C-132`/`C-133` |

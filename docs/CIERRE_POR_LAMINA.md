...
| **9** · Directa · Status M2 | `L-038` | ⛔ ↕ | **9 tokens, no 8 — medido el 25/08 contra la plantilla viva**, y **8 tienen fila**. ⭐ **El numerador coincide y el denominador no** (−9,6 % en enviados/entregados, con el numerador quieto) → falta **decidir** de qué universo sale el denominador. ⭐ **`m2_campanias` se conserva y su número NO se usa** (usuario, 25/08): el banner de *Campañas* lo escribe el equipo, y el motivo quedó escrito **en su `notas`**, porque una fila que no se usa no entra a `FALTANTES` y es invisible desde la hoja. ⛔ **`m2_camp_lista` es el único sin fila, y está FRENADO por una decisión, no por trabajo**: la decisión es publicar los nombres **crudos** y `opLISTA` publica **contra un catálogo** por diseño — y no hay catálogo de nombres de campaña de M2 (la columna A de `digital/Directa Mail` es `ID Cuentas`). La pregunta exacta está en `PENDIENTES`. ⚠ **Y la caja está medida y contradice lo que se suponía**: `autofit = NONE` y alto **24**, no `SHAPE_AUTOFIT` — así que **no crece**. El token comparte caja con el literal «Campañas» | 25/08 |
...
| *(orden 8)* · «1 a 1» | `L-053` | 🟡 | ⭐⭐ **VALIDADA MARCADOR POR MARCADOR el 26/08/2026**, sobre `3487-AGOJDGAG` / `agosto_14_20`, con las tres lecturas al lado —fixture 20/08, deck 22/08, base viva 26/08— y las **identidades internas cerrando al dígito en las tres** (partes = total en impresiones, visualizaciones y clics). **36 tokens: 28 cableados + 8 sin fila.** ⭐ Control positivo por el mismo camino: `V-21` **17.401** y `V-23` **25.099**, más `V-22` **496** y `V-24` **778** — 4 de 4. ⭐ **Lo que se arregló el 26/08:** `u1_total_clics` publicaba **1.472** bajo un rótulo `PRE + POST` y ahora publica **2.464** (`R-33`, deroga `R-28`); `u1_total_vistas` pasó al mismo corte aunque **daba bien**, porque acertaba sólo porque las PRE de esta cuenta traen 0 visualizaciones; salió el `_revisar` de los **24**; y la fecha se publica **sin año** (`12/08`). ⛔ **`u1_total_alcance` y `u1_total_frecuencia` quedan SIN CABLEAR y eso NO es un faltante de la lámina: EL DATO NO EXISTE TODAVÍA** (decisión del usuario, 26/08). Van declarados acá para que no cuenten en el conteo de `D-38`. ⚠ Los seis `u1_bench_*` ya están en `TOKENS_EQUIPO_JM_` — los escribe una persona. ⛔ **Quedan dos ⛔ del 22/08 sin explicar y no se investigaron** (decisión: la tabla primero): `ecv_barrio` y `ecv_fecha` resuelven `ok` hoy y el deck del 22/08 los publicó `/////`. **El ✅ lo pone el usuario** | 26/08 |

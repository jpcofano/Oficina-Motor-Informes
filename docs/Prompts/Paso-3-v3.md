...
**No cablear los 200 marcadores.** Elegir **una slide de JM** y completar `base_id`,
`campo_logico` y `operacion` de sus tokens solamente (5–10). Sugerencia: la slide 5
(ECV — alcance por herramienta), que usa una sola base y mezcla `SUMA` con `PCT`.

Ítem de menú **"Calcular marcadores de prueba"** → tabla con marcador · valor ·
valor_formateado · estado · traza.

**Prueba específica de la Parte B**, que es lo que este paso agrega y hay que ejercitar:
...
- ninguno de los dos, `CONFIG` vacío → **calcula la semana de `R-11`**, siete días,
  viernes a jueves. Reportar las dos fechas que devuelve.

...
| `REUNIONES` | curada a mano + `cargarTemarioReuniones_`, por **tres entradas** desde el 20/08: el ítem de menú "Cargar temario", la llamada por API `cargarTemario(texto, periodoId)` (Paso 2.14) y **el botón del panel** (`panel_cargarTemario`, `2026-08-19_2`) — las tres pasan por el mismo cargador, que **desde el 20/08 saltea lo que ya existe** en vez de hacer append ciego | `cargarTemarioReuniones_` para las filas, **y desde el 27/08 `escribirCuentasAncladas_` para `id_cuenta`** (`Union.gs`, vía `curarCamposReuniones_`) | ✅ un solo escritor, dos puertas. Desde el Paso 2.15 Parte B **el período es obligatorio en las dos**: `cargarTemario` valida contra `PERIODOS` y falla explícito antes de escribir (`D-19`) |
| `REUNIONES.id_cuenta` | ⭐ **el ANCLAJE**, desde el 27/08 (`D-49`): `escribirCuentasAncladas_` escribe la cuenta que resolvió, **una sola vez fuera del bucle** y por clave `texto_original` | `curarCamposReuniones_`, que escribe **sólo los campos nombrados** y no pasa por `upsertPorClave_` — o sea que **no blanquea las demás columnas** | ⚠ **escribe sólo si la celda está VACÍA**: una cuenta declarada a mano gana y no se pisa. Los de **baja confianza NO se escriben** — declarar una cuenta que el motor considera dudosa convertiría una duda en un hecho |
